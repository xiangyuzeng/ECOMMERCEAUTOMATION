...
- ✅ 系统在任何电脑上的运行环境完全一致
- ✅ 方便在服务器上部署
- ✅ 自动包含 Playwright 和 Chromium 浏览器
...
> 💡 **提示**：Docker Desktop 是免费的。安装后它会在系统后台运行。
...
> ⚠️ **注意**：Docker 容器使用 `network_mode: host`，因此可以直接访问本地运行的 AdsPower（localhost:50325）。这意味着 AdsPower 需要在宿主机（你的电脑）上运行，不在 Docker 容器内。

### 11.4 Docker 镜像结构

系统使用多阶段构建（multi-stage build），Dockerfile 结构如下：

| 阶段 | 说明 |
|------|------|
| **Stage 1: base** | Python 3.11 + Node.js 20 + Playwright/Chromium 依赖 + 中文字体（fonts-noto-cjk） |
| **Stage 2: dashboard-builder** | 构建 Next.js 仪表盘（npm install + npm run build） |
| **Stage 3: final** | 最终生产镜像，包含 Python 环境 + 构建好的仪表盘 |
...
Docker 配置（docker-compose.yml）中使用了以下数据卷（Volume），将容器内的目录映射到主机：

| 主机目录 | 容器目录 | 说明 | 读写权限 |
|----------|----------|------|----------|
| `./data` | `/app/data` | 多产品数据存储 | 读写 |
| `./config.json` | `/app/config.json` | 配置文件 | 只读 |
| `./inputs` | `/app/inputs` | 输入数据文件 | 读写 |
| `./outputs` | `/app/outputs` | 输出报告 | 读写 |
| `./processed` | `/app/processed` | 处理后的 JSON 数据 | 读写 |
...
启动延迟：15秒
重试次数：3次
检查方式：curl http://localhost:3000/
```

如果仪表盘无法访问，Docker 会自动重启容器（`restart: unless-stopped`）。
